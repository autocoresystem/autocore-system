...
export default function Dashboard() {
  ...
  const filteredClients = useMemo(() => {
    return clients.filter((client) =>
      `${client.name} ${client.contact} ${client.type} ${client.status}`
        .toLowerCase()
  ...
  const statusColor = (status) => {
    if (
      ["Pagada", "Completado", "Activo", "Al día", "Disponible", "Confirmado", "Listo"].includes(
        status
      )
    )
        ...
        <div className="flex gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="rounded-2xl border border-white/10 bg-black/40 py-2.5 pl-10 pr-4 text-sm text-white outline-none placeholder:text-zinc-500"
            />
          </div>
          <button
            onClick={() => {
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/10">
        <div className="min-w-[700px]">
          ...
          {filteredClients.map((client) => (
            <div
              key={client.name}
              className="grid grid-cols-5 border-t border-white/10 px-4 py-4 text-sm text-zinc-200"
            >
  ...
  const renderBilling = () => (
    <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
      <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-6">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <p className="text-sm text-zinc-400">Facturación</p>
            <h2 className="text-xl font-bold tracking-tight">Facturas recientes</h2>
          </div>
          <button
            ...
            <div key={invoice.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-500">Vencimiento: {invoice.due}</p>
                </div>
                <div className="text-right">
                  <p className="font-black tracking-tight text-white">{invoice.amount}</p>
                  <p className={`text-sm ${statusColor(invoice.status)}`}>{invoice.status}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="space-y-6">
        <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-6">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <p className="text-sm text-zinc-400">Cobros</p>
              <h2 className="text-xl font-bold tracking-tight">Resumen financiero</h2>
            </div>
          ...
          <div className="space-y-3">
            {["Cliente contactado", "Cotización enviada", "Factura generada", "Pago procesado"].map(
              (step, i) => (
                ...
                >
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-red-500/10 text-sm font-bold text-red-300">
                    {i + 1}
                  </div>
  ...
  const renderVehicles = () => (
    <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-6">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <p className="text-sm text-zinc-400">Gestión de unidades</p>
          <h2 className="text-xl font-bold tracking-tight">Inventario de vehículos</h2>
        </div>
        <div className="flex gap-3">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {vehicles.map((vehicle) => (
          <div key={vehicle.plate} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <CarFront className="mb-4 h-5 w-5 text-red-500" />
            <p className="font-semibold text-white">{vehicle.unit}</p>
            <p className="mt-1 text-sm text-zinc-500">Placa: {vehicle.plate}</p>
            <div className="mt-4 flex items-center justify-between text-sm">
              <span className="text-zinc-400">{vehicle.type}</span>
              <span className={statusColor(vehicle.status)}>{vehicle.status}</span>
            </div>
          </div>
        ))}
      </div>
  ...
  const renderWorkshop = () => (
    <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
      <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-6">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <p className="text-sm text-zinc-400">Taller</p>
            <h2 className="text-xl font-bold tracking-tight">Órdenes de trabajo</h2>
          </div>
          <button className="inline-flex items-center gap-2 rounded-2xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white">
            <Plus className="h-4 w-4" /> Nueva orden
          </button>
        </div>
        <div className="space-y-3">
          {workshopJobs.map((job) => (
            <div key={job.ticket} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-white">
                    {job.ticket} · {job.unit}
      ...
      <div className="space-y-6">
        <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-6">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <h2 className="text-xl font-bold tracking-tight">Agenda del día</h2>
            </div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={hour} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                <p className="text-sm text-zinc-500">{hour}</p>
                <p className="mt-1 font-medium text-white">{task}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold tracking-tight">Servicios rápidos</h3>
          </div>
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {reportCards.map((card) => {
          ...
          return (
            <div key={card.title} className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-5">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-6">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <p className="text-sm text-zinc-400">Reportes</p>
              <h2 className="text-xl font-bold tracking-tight">Indicadores clave</h2>
            </div>
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="mx-auto flex min-h-screen max-w-7xl items-center px-6 py-10 lg:px-8">
          <div className="grid w-full gap-8 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <motion.div
              {...fadeUp}
              className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-[0_30px_100px_rgba(0,0,0,0.4)] backdrop-blur-2xl lg:p-8"
            >
              <div className="mb-6 flex items-center gap-3">
                <img
                  src="/autocore-logo.png"
                  alt="AutoCore"
                  className="h-12 w-auto drop-shadow-[0_0_20px_rgba(255,0,0,0.6)]"
                />
                <div>
              ...
              <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/80 p-5">
                ...
                <div className="mt-6 space-y-4">
                  <div>
                    ...
                    <input
                      type="email"
                      defaultValue="demo@autocore.com"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 pr-12 text-white outline-none placeholder:text-zinc-500"
                      />
                      <button
                        type="button"
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left transition ${
                      active
                        ? "border border-red-500/20 bg-red-500/10 text-white"
                        : "border border-transparent bg-white/[0.03] text-zinc-300 hover:bg-white/[0.06]"
                    }`}
                  >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-[2rem] border border-white/10 bg-zinc-950 p-6 shadow-[0_30px_100px_rgba(0,0,0,0.5)]">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button className="rounded-2xl bg-red-600 px-5 py-3 font-semibold text-white">
                Guardar factura
              </button>
            </div>
          </div>
